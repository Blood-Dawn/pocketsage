...
- Persistence: SQLModel tables for Account, Category, Budget/BudgetLine, Transaction, Habit/HabitEntry, Liability, Holding, AppSetting. Relationships include Transaction -> Category, Budget -> BudgetLine, Habit -> HabitEntry, Holding -> Account.
- Services: budgeting (NotImplemented stubs), debts payoff calculators + chart helper, CSV import/export (ledger/portfolio), reporting/chart generation, optional filesystem watcher, simple in-memory job runner, and desktop-friendly admin tasks in `services/admin_tasks.py` (demo seed/export/retention).
...
## Run Mode & Quickstart
- Desktop (Flet): create venv, `pip install -e ".[dev]"`, copy `.env`, run `python run_desktop.py`. Shortcuts: `Ctrl+N` new transaction, `Ctrl+Shift+H` new habit, `Ctrl+1..7` navigation, quick actions in the app bar; month selector refreshes Budgets/Reports.
...
- Packaging: `make package` runs `flet pack run_desktop.py`; platform scripts `bash scripts/build_desktop.sh` (Linux/macOS) or `scripts\\build_desktop.bat` (Windows, prompts to delete `build` unless run with `--delete-build`) emit binaries to `dist/`.
...
- Admin tasks live in `services/admin_tasks.py`: `run_demo_seed(session_factory)` (per-user, used at login/guest creation) and `run_export(output_dir, session_factory, user_id)` with `EXPORT_RETENTION = 5`. Desktop Settings/Reports screens call these helpers directly.
...
## Testing & QA
- Tooling: pytest, ruff, black, bandit, safety; CI runs tests (Py3.11/3.12), lint, security, build check, coverage upload (target 40%+; module goals higher), with pip caching and parallel pytest via xdist (suite currently green: 123 tests).
...
- Manual QA: run desktop smoke (nav + shortcuts, month selector refresh, ledger add/delete/export/import, debts CRUD/payments + chart, portfolio CRUD/import/export + chart, reports exports, admin seed/reset/roles).
- Testing infrastructure summary: extensive suites for debts (snowball/avalanche, freed minimum rollover), habits (streaks, upsert), CSV import (mapping, validation, UTF-8, idempotency), money representation (float vs Decimal), integration workflows, repository CRUD/filtering. CI workflow defined in `.github/workflows/ci.yml` with lint/security/build steps. Remaining work: budgeting UI/creation, watcher flows, export destination picker, admin delete/reset flows, UI automation.
...
- Current state (2025-11-20): Ledger filters/export fixed; month selector refreshes Budgets/Reports; debts have CRUD + payments + schedule + chart; portfolio CRUD/import/export + allocation chart refresh; reports implemented (full export, monthly spending, YTD CSV, debt payoff CSV+chart); per-user seeding/guest purge wired. Budget creation/editing, habit creation/archive, admin delete/reset, export destination picker, watcher, and advanced reports remain open.
- Feature status: ledger/budgets/habits/debts/portfolio/reports/settings/admin routes fully wired; budget UI still read-only; habits lack create/archive UX; admin lacks delete/reset password; watcher not enabled; exports go to `instance/exports`.
- TODO backlog (see `TODO.md`): budgets creation, habits creation/archive/reminders, debt payment history to ledger, portfolio filters/market value, admin delete/reset/full-export/restore, import mapping suggestions + watcher, export destination picker, UI automation/perf tests.
...
- Navigation uses centralized controllers with transitions disabled; Help tab works.
- Shared FilePicker overlays power ledger and portfolio CSV imports; Settings buttons trigger import, demo seed, reset, backup/restore, and watched-folder auto-import flows with snack feedback.
- Settings includes explicit "Reset Demo Data" alongside "Run Demo Seed"; Help/CSV help buttons route to `/help`.
- Month selector refreshes Budgets/Reports; debts and portfolio now have full CRUD + charts; reports cards produce exports (spending PNG, YTD CSV, debt payoff CSV/PNG, category trend, cashflow by account, combined bundle).
...
- TODO: consolidated owner list in `TODO.md` (budgets/habits/portfolio/admin/imports/report polish, watcher, QA).
