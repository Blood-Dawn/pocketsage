...
def test_handle_navigation_selection_updates_route() -> None:
    ...
    controllers.handle_nav_selection(page, 1)  # /ledger
...
def test_file_picker_result_triggers_ledger_import(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    def fake_import_ledger_transactions(*, csv_path: Path, session_factory, mapping=None):
        calls["ledger"] = csv_path
        return 2

    monkeypatch.setattr(controllers.importers, "import_ledger_transactions", fake_import_ledger_transactions)

    ctx = SimpleNamespace(session_factory=lambda: None, file_picker=None, file_picker_mode=None)
    page = _PageSpy()
...
def test_file_picker_result_triggers_portfolio_import(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    calls: dict[str, Path] = {}

    def fake_import_portfolio_holdings(*, csv_path: Path, session_factory):
        calls["portfolio"] = csv_path
        return 3

    monkeypatch.setattr(controllers.importers, "import_portfolio_holdings", fake_import_portfolio_holdings)
...
def test_run_and_reset_demo_seed_show_feedback(session_factory):
    page = _PageSpy()
    ctx = SimpleNamespace(session_factory=session_factory, file_picker=None, file_picker_mode=None)
