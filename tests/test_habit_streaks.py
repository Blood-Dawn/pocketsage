...
class TestCurrentStreak:
    ...
    def test_no_entries_returns_zero_streak(self, session_factory, habit_factory):
        ...
        streak = repo.get_current_streak(habit.id, user_id=uid)
        assert streak == 0
    ...
    def test_single_entry_today_returns_one(self, session_factory, habit_factory, db_session):
        ...
        # Add entry for today
        today = date.today()
        entry = HabitEntry(habit_id=habit.id, occurred_on=today, value=1, user_id=habit.user_id)
    ...
    def test_consecutive_days_returns_correct_streak(
        ...
        for i in range(7):
            ...
            entry = HabitEntry(habit_id=habit.id, occurred_on=day, value=1, user_id=habit.user_id)
            db_session.add(entry)
        db_session.commit()
    ...
    def test_gap_breaks_streak(self, session_factory, habit_factory, db_session):
        ...
        db_session.add(
            HabitEntry(habit_id=habit.id, occurred_on=today, value=1, user_id=habit.user_id)
        )
        db_session.add(
            HabitEntry(
                habit_id=habit.id,
...
class TestLongestStreak:
    ...
    def test_no_entries_returns_zero(self, session_factory, habit_factory):
        ...
        streak = repo.get_longest_streak(habit.id, user_id=uid)
        assert streak == 0
    ...
    def test_single_entry_returns_one(self, session_factory, habit_factory, db_session):
        ...
        entry = HabitEntry(
            habit_id=habit.id, occurred_on=date.today(), value=1, user_id=habit.user_id
        )
        db_session.add(entry)
        db_session.commit()
    ...
    def test_multiple_streaks_returns_longest(self, session_factory, habit_factory, db_session):
        ...
        for i in range(3):
            ...
            db_session.add(
                HabitEntry(habit_id=habit.id, occurred_on=day, value=1, user_id=habit.user_id)
            )
    ...
    def test_current_streak_can_be_longest(self, session_factory, habit_factory, db_session):
        ...
        db_session.add(
            HabitEntry(
                habit_id=habit.id, occurred_on=date(2024, 1, 1), value=1, user_id=habit.user_id
            )
        )
        db_session.add(
            HabitEntry(
                habit_id=habit.id, occurred_on=date(2024, 1, 2), value=1, user_id=habit.user_id
            )
        )
        ...
        longest = repo.get_longest_streak(habit.id, user_id=uid)
        current = repo.get_current_streak(habit.id, user_id=uid)
    ...
    def test_zero_value_entries_ignored(self, session_factory, habit_factory, db_session):
        ...
        for i in range(5):
            ...
            db_session.add(
                HabitEntry(habit_id=habit.id, occurred_on=day, value=value, user_id=habit.user_id)
            )
    ...
    def test_non_consecutive_dates_creates_multiple_streaks(
        ...
        db_session.add(
            HabitEntry(
                habit_id=habit.id, occurred_on=date(2024, 1, 3), value=1, user_id=habit.user_id
            )
        )
...
class TestHabitEntryUpsert:
    ...
    def test_upsert_creates_new_entry(self, session_factory, habit_factory):
        ...
        upserted = repo.upsert_entry(entry, user_id=uid)
    ...
    def test_upsert_updates_existing_entry(self, session_factory, habit_factory, db_session):
        ...
        entry1 = HabitEntry(habit_id=habit.id, occurred_on=today, value=1, user_id=habit.user_id)
        db_session.add(entry1)
        db_session.commit()

        # Upsert with different value
        entry2 = HabitEntry(habit_id=habit.id, occurred_on=today, value=3, user_id=habit.user_id)
        upserted = repo.upsert_entry(entry2, user_id=uid)
    ...
    def test_upsert_different_dates_creates_separate_entries(self, session_factory, habit_factory):
        ...
        # Upsert for today
        entry1 = HabitEntry(habit_id=habit.id, occurred_on=today, value=1, user_id=habit.user_id)
        repo.upsert_entry(entry1, user_id=uid)

        # Upsert for yesterday
        entry2 = HabitEntry(
            habit_id=habit.id, occurred_on=yesterday, value=1, user_id=habit.user_id
        )
        repo.upsert_entry(entry2, user_id=uid)

        # Should have 2 separate entries
        entries = repo.get_entries_for_habit(habit.id, yesterday, today, user_id=uid)
...
class TestGetEntriesForHabit:
    ...
    def test_get_entries_in_range(self, session_factory, habit_factory, db_session):
        ...
        entries = repo.get_entries_for_habit(habit.id, start, end, user_id=uid)
    ...
    def test_entries_ordered_by_date(self, session_factory, habit_factory, db_session):
        ...
        for d in dates:
            entry = HabitEntry(habit_id=habit.id, occurred_on=d, value=1, user_id=habit.user_id)
            db_session.add(entry)
        db_session.commit()
