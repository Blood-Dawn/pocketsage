...
def _calculate_schedule(
    *, debts: Iterable[DebtAccount], surplus: float
) -> list[dict]:
    ...
    # Calculate the total of all minimum payments to add to the snowball/avalanche payment
    ...
    while any(d['balance'] > 0 for d in debts):
        ...
        if payoff_schedule:
            ...
            extra_payment_pool += sum(last_row.get(f'min_payment_{d["id"]}', 0) for d in debts if d['balance'] == 0)
        ...
        for i, debt in enumerate(debts):
            if debt['balance'] > 0:
                ...
                monthly_interest_float = float(monthly_interest.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
